...
function showDead(used) {
  ...
  $("wheel-dead-text").textContent = used
    ? "Ты уже купил билет по этой скидке. Устройство мы видим по IP — второй раз колесо не прокрутить."
    : "Твоё устройство мы видим по IP: прокрут был один и он уже потрачен. Но билеты на месте — забирай без скидки, депозит всё равно возвращается баром.";
}
...
// «Забрать скидку» — сразу к билетам, скидка уже применена к ценам
$("wheel-take").addEventListener("click", () => {
