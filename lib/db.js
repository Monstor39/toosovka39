...
export function load() {
  ...
  if (!db.spins) db.spins = {}; // прокруты колеса CRAZY DUBAI по клиентам
...
function codeRecord(order) {
  ...
  return {
    ...
    manual: false, // куплен на сайте, а не выдан вручную
...
export function history(limit = 15) {
  return Object.entries(db.codes)
    ...
    .map(([code, c]) => ({ code, ticket: c.ticket, ticketName: c.ticketName, contact: c.contact, usedAt: c.usedAt, deposit: c.deposit, noDeposit: c.noDeposit, paidRub: c.paidRub, wheel: c.wheel, promo: c.promo, manual: c.manual }))
...
export function insideList() {
  return Object.entries(db.codes)
    ...
    .map(([code, c]) => ({ code, ticket: c.ticket, contact: c.contact, usedAt: c.usedAt, deposit: c.deposit, noDeposit: c.noDeposit, paidRub: c.paidRub, wheel: c.wheel, promo: c.promo, manual: c.manual }))
...
export function counts() {
  ...
  // Колесо CRAZY DUBAI: сколько крутили, сколько из них купили
