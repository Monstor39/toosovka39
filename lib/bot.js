...
// Клавиатура зависит от того, кто пишет: кнопка выдачи кодов — только владельцу
function keyboardFor(id) {
  ...
  if (isOwner(id)) rows.push([{ text: KB_GIVE }]);
...
const ICONS = { vip: "🥂", standard: "🎟", entry: "🎫", manual: "🎁" };
...
function discountNote(info) {
  ...
  if (info.manual) parts.push("выдан вручную");
...
function depositLine(info) {
  if (info.noDeposit) {
    ...
    const kind = info.manual ? "выдан вручную" : "входной билет";
...
function barCard(code, info, note = "", owner = false) {
  ...
  if (info.noDeposit) {
    const kind = info.manual ? "код выдан без депозита" : "это входной билет";
    return {
      text:
...
async function handleCallback(cb) {
  ...
  // ---------- Выдача кодов гостям и тестовые коды: только владелец ----------
  if (action === "gv" || action === "gvx" || action === "gvmore" || action === "gcash" || action === "test") {
...
function statsText() {
  ...
  // Рядом с количеством — сумма, на которую этих билетов купили.
  const perTicket = tickets
    .map(
      (t) =>
        `${ticketIcon(t.id)} ${t.name}: <b>${db.soldCount(t.id)}</b> шт` +
        `${t.limit ? ` из ${t.limit}` : ""} · <b>${fmtMoney(db.soldSum(t.id))}</b>`
    )
    .join("\n");
  const c = db.counts();
  return (
    ...
    perTicket +
    "\n\n" +
    `💵 <b>Выручка с сайта: ${fmtMoney(c.revenue)}</b> (${c.soldSite} билетов)\n` +
    (c.manualCount
