...
function statsText() {
  const perTicket = tickets
    .map((t) => `${t.name}: продано <b>${db.soldCount(t.id)}/${t.limit}</b>`)
    .join("\n");
  const c = db.counts();
  return (
...
function insidePanel() {
  ...
  const buttons = shown.map((r) => [
    {
      text: `${r.ticket === "vip" ? "🥂" : "🎟"} ${r.code} · ${r.contact}`,
...
function historyText() {
  ...
  const lines = rows.map(
    (r) => `${r.ticket === "vip" ? "🥂" : "🎟"} <b>${r.code}</b> — ${fmtTime(r.usedAt)}`
  );
