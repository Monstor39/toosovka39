...
const ICONS = { vip: "🥂", standard: "🎟", entry: "🎫" };
const ticketIcon = (id) => ICONS[id] || "🎟";
function ticketLabel(id) {
  return `${ticketIcon(id)} <b>${ticketById[id]?.name || "Билет"}</b>`;
}
...
// Депозит человека — сколько он реально заплатил (по промокоду или скидке
// с колеса меньше). Охране это нужно на входе: у одного депозит 1 200 ₽,
// у другого 600 ₽, а у входного билета за 200 ₽ депозита нет вообще.
...
// Откуда взялась скидка — чтобы охрана не спорила с гостем
function discountNote(info) {
  const parts = [];
  if (info.promo) parts.push(`промокод ${info.promo}`);
  if (info.wheel) parts.push(`колесо −${info.wheel}%`);
  return parts.length ? ` · ${parts.join(" · ")}` : "";
}
function depositLine(info) {
  if (info.noDeposit) {
    const paid = info.paidRub ?? ticketById[info.ticket]?.price;
    return `💰 Депозит на баре: <b>нет</b> (входной билет${paid ? `, оплачено ${fmtMoney(paid)}` : ""})\n`;
  }
  const price = depositOf(info);
  if (price === null) return "";
  return `💰 Депозит на баре: <b>${price === 0 ? "нет (бесплатный вход)" : fmtMoney(price)}</b>${discountNote(info)}\n`;
}
...
function barCard(code, info, note = "") {
  if (info.noDeposit) {
    return {
      text:
        `${note}🎫 <b>${code}</b> · ${ticketById[info.ticket]?.name || "билет"}\n` +
        `💰 Депозита на баре <b>нет</b> — это входной билет, бар оплачивается отдельно.`,
  ...
  const text =
    ...
    `💰 Депозит: <b>${fmtMoney(info.deposit ?? 0)}</b>${discountNote(info)}\n` +
...
async function sendBarCard(chatId, code, info, note = "") {
  const card = barCard(code, info, note);
...
async function handleCallback(cb) {
  ...
  if (action === "sp" || action === "undo" || action === "bar" || action === "spx") {
    ...
    if (action === "bar") {
      ...
      await sendBarCard(chatId, code, info);
    ...
    const card = barCard(code, db.getCode(code), note);
...
function statsText() {
  ...
  return (
    ...
    `🎡 Колесо: крутили <b>${c.wheelSpins}</b>, купили со скидкой <b>${c.wheelSold}</b>\n\n` +
    `🍸 <b>Бар</b>\n` +
...
async function handleMessage(msg) {
  ...
  if (text === KB_BAR || text === "/bar") {
    await api("sendMessage", { chat_id: chatId, parse_mode: "HTML", text: barListText(), reply_markup: guardKeyboard });
    return;
  }
  if (text === KB_SPENDS || text === "/spends") {
    await api("sendMessage", { chat_id: chatId, parse_mode: "HTML", text: spendHistoryText(), reply_markup: guardKeyboard });
    return;
  }
...
async function doSpend(chatId, code, amount, byId) {
  ...
  const texts = {
    ...
    no_deposit: `🎫 У кода <b>${code}</b> нет депозита — это входной билет, бар оплачивается отдельно.`,
  ...
  if (info && info.used && !info.noDeposit && r.reason !== "not_found") await sendBarCard(chatId, code, info);
}
